...
fun Element.typeIsEnumeratedCode(valueSetMap: Map<String, ValueSet>): Boolean {
  val valueSet = valueSetMap[getValueSetUrl()] ?: return false
...
fun Element.getSurrogatePropertyNameTypeDefaultValueList(
  ...
  getContentReferenceType(enclosingClassName.packageName)?.let {
    return listOf(
      Triple(getElementName(), it.withCardinalityInSurrogate(this), getDefaultValueInSurrogate())
    )
  }
    ...
    listOf(
      Triple(
        ...
        FhirPathType.getFromFhirTypeCode(type.code)!!
          ...
          // possible for the the element to only have extensions in the companion element (prefixed
          // with an underscore).
          .copy(nullable = true),
        getDefaultValueInSurrogate(),
      ),
      Triple(
        ...
        ClassName(enclosingClassName.packageName, "Element")
          .withCardinalityInSurrogate(this)
          // The companion element field prefixed with an underscore should always be nullable,
          // whatever the cardinality, since it is always possible to omit this field entirely in
          // JSON.
          .copy(nullable = true),
        // The default value of the companion element field prefixed with an underscore should
        // always be `null`, for the same reason as above.
        "null",
      ),
    )
  } else {
...
fun Element.getDefaultValueInSurrogate(): String? {
  ...
  if ((type?.size ?: 0) > 1) {
    // Choice of type should always be nullable since there is no guarantee that any of the
    // choices is the provided type, and therefore should always have default value `null`.
    return "null"
  }
  return null
}
