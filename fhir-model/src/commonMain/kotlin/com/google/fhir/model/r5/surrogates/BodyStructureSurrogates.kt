    ...
    public fun fromModel(
      ...
      with(model) {
        BodyStructureIncludedStructureBodyLandmarkOrientationDistanceFromLandmarkSurrogate(
          ...
          device = this@with.device.takeUnless { it.isEmpty() },
    ...
    public fun fromModel(
      ...
      with(model) {
        BodyStructureIncludedStructureBodyLandmarkOrientationSurrogate(
          ...
          extension = this@with.extension.takeUnless { it.isEmpty() },
          modifierExtension = this@with.modifierExtension.takeUnless { it.isEmpty() },
          landmarkDescription = this@with.landmarkDescription.takeUnless { it.isEmpty() },
          clockFacePosition = this@with.clockFacePosition.takeUnless { it.isEmpty() },
          distanceFromLandmark = this@with.distanceFromLandmark.takeUnless { it.isEmpty() },
          surfaceOrientation = this@with.surfaceOrientation.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(
      ...
      with(model) {
        BodyStructureIncludedStructureSurrogate(
          ...
          bodyLandmarkOrientation = this@with.bodyLandmarkOrientation.takeUnless { it.isEmpty() },
          spatialReference = this@with.spatialReference.takeUnless { it.isEmpty() },
          qualifier = this@with.qualifier.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class BodyStructureSurrogate(
  ...
  public fun toModel(): BodyStructure =
    BodyStructure(
      ...
      language =
        Code.ofNullable(
          this@BodyStructureSurrogate.language,
          this@BodyStructureSurrogate._language,
        ),
      ...
      active =
        R5Boolean.ofNullable(
          this@BodyStructureSurrogate.active,
          this@BodyStructureSurrogate._active,
        ),
    ...
    public fun fromModel(model: BodyStructure): BodyStructureSurrogate =
      with(model) {
        BodyStructureSurrogate(
          ...
          includedStructure = this@with.includedStructure.takeUnless { it.isEmpty() },
          excludedStructure = this@with.excludedStructure.takeUnless { it.isEmpty() },
          description = this@with.description?.value,
          _description = this@with.description?.toElement(),
          image = this@with.image.takeUnless { it.isEmpty() },
