...
internal data class SubscriptionTopicResourceTriggerSurrogate(
  ...
  public var resource: KotlinString?,
      ...
      supportedInteraction =
        if (
          ...
          (this@SubscriptionTopicResourceTriggerSurrogate.supportedInteraction
            ...
            .map { (value, element) ->
              Enumeration.of(
                value.let { com.google.fhir.model.r5.SubscriptionTopic.MethodCode.fromCode(it!!) },
                element,
              )
            }
            .toMutableList()
    ...
    public fun fromModel(
      ...
      with(model) {
        SubscriptionTopicNotificationShapeSurrogate(
          ...
          _resource = this@with.resource.toElement(),
          include =
            this@with.include.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _include =
            this@with.include
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ...
              ?.toMutableList(),
          revInclude =
            this@with.revInclude.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _revInclude =
...
internal data class SubscriptionTopicSurrogate(
  ...
  public fun toModel(): SubscriptionTopic =
    SubscriptionTopic(
      ...
      versionAlgorithm =
        SubscriptionTopic.VersionAlgorithm?.fromNullable(
          R5String.ofNullable(
      ...
      name =
        R5String.ofNullable(
          this@SubscriptionTopicSurrogate.name,
          this@SubscriptionTopicSurrogate._name,
        ),
      title =
        R5String.ofNullable(
          this@SubscriptionTopicSurrogate.title,
          this@SubscriptionTopicSurrogate._title,
        ),
      derivedFrom =
        if (
    ...
    public fun fromModel(model: SubscriptionTopic): SubscriptionTopicSurrogate =
      with(model) {
        SubscriptionTopicSurrogate(
          ...
          resourceTrigger = this@with.resourceTrigger.takeUnless { it.isEmpty() },
          eventTrigger = this@with.eventTrigger.takeUnless { it.isEmpty() },
          canFilterBy = this@with.canFilterBy.takeUnless { it.isEmpty() },
          notificationShape = this@with.notificationShape.takeUnless { it.isEmpty() },
        )
      }
  }
}
