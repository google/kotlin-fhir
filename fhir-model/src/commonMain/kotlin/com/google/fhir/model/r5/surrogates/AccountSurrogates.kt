    ...
    public fun fromModel(model: Account.Diagnosis): AccountDiagnosisSurrogate =
      with(model) {
        AccountDiagnosisSurrogate(
          ...
          packageCode = this@with.packageCode.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class AccountBalanceSurrogate(
  ...
  public fun toModel(): Account.Balance =
    Account.Balance(
      ...
      estimate =
        R5Boolean.ofNullable(
          this@AccountBalanceSurrogate.estimate,
          this@AccountBalanceSurrogate._estimate,
        ),
...
internal data class AccountSurrogate(
  ...
  public fun toModel(): Account =
    Account(
      ...
      implicitRules =
        Uri.ofNullable(this@AccountSurrogate.implicitRules, this@AccountSurrogate._implicitRules),
      language = Code.ofNullable(this@AccountSurrogate.language, this@AccountSurrogate._language),
      ...
      name = R5String.ofNullable(this@AccountSurrogate.name, this@AccountSurrogate._name),
      ...
      description =
        Markdown.ofNullable(this@AccountSurrogate.description, this@AccountSurrogate._description),
    ...
    public fun fromModel(model: Account): AccountSurrogate =
      with(model) {
        AccountSurrogate(
          ...
          coverage = this@with.coverage.takeUnless { it.isEmpty() },
          owner = this@with.owner,
          description = this@with.description?.value,
          _description = this@with.description?.toElement(),
          guarantor = this@with.guarantor.takeUnless { it.isEmpty() },
          diagnosis = this@with.diagnosis.takeUnless { it.isEmpty() },
          procedure = this@with.procedure.takeUnless { it.isEmpty() },
          relatedAccount = this@with.relatedAccount.takeUnless { it.isEmpty() },
          currency = this@with.currency,
          balance = this@with.balance.takeUnless { it.isEmpty() },
