...
internal data class SubstanceDefinitionMoietySurrogate(
  ...
  public fun toModel(): SubstanceDefinition.Moiety =
    SubstanceDefinition.Moiety(
      ...
      amount =
        SubstanceDefinition.Moiety.Amount?.fromNullable(
    ...
    public fun fromModel(
      ...
      with(model) {
        SubstanceDefinitionCharacterizationSurrogate(
          ...
          `file` = this@with.`file`.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class SubstanceDefinitionPropertySurrogate(
  ...
  public fun toModel(): SubstanceDefinition.Property =
    SubstanceDefinition.Property(
      ...
      `value` =
        SubstanceDefinition.Property.Value?.fromNullable(
    ...
    public fun fromModel(
      ...
      with(model) {
        SubstanceDefinitionStructureSurrogate(
          ...
          technique = this@with.technique.takeUnless { it.isEmpty() },
          sourceDocument = this@with.sourceDocument.takeUnless { it.isEmpty() },
          representation = this@with.representation.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(model: SubstanceDefinition.Name): SubstanceDefinitionNameSurrogate =
      with(model) {
        SubstanceDefinitionNameSurrogate(
          ...
          synonym = this@with.synonym.takeUnless { it.isEmpty() },
          translation = this@with.translation.takeUnless { it.isEmpty() },
          official = this@with.official.takeUnless { it.isEmpty() },
          source = this@with.source.takeUnless { it.isEmpty() },
        )
      }
  }
...
internal data class SubstanceDefinitionRelationshipSurrogate(
  ...
  public fun toModel(): SubstanceDefinition.Relationship =
    SubstanceDefinition.Relationship(
      ...
      substanceDefinition =
        SubstanceDefinition.Relationship.SubstanceDefinition?.fromNullable(
      ...
      amount =
        SubstanceDefinition.Relationship.Amount?.fromNullable(
    ...
    public fun fromModel(
      ...
      with(model) {
        SubstanceDefinitionSourceMaterialSurrogate(
          ...
          part = this@with.part,
          countryOfOrigin = this@with.countryOfOrigin.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(model: SubstanceDefinition): SubstanceDefinitionSurrogate =
      with(model) {
        SubstanceDefinitionSurrogate(
          ...
          grade = this@with.grade.takeUnless { it.isEmpty() },
          description = this@with.description?.value,
          _description = this@with.description?.toElement(),
          informationSource = this@with.informationSource.takeUnless { it.isEmpty() },
          note = this@with.note.takeUnless { it.isEmpty() },
          manufacturer = this@with.manufacturer.takeUnless { it.isEmpty() },
          supplier = this@with.supplier.takeUnless { it.isEmpty() },
          moiety = this@with.moiety.takeUnless { it.isEmpty() },
          characterization = this@with.characterization.takeUnless { it.isEmpty() },
          `property` = this@with.`property`.takeUnless { it.isEmpty() },
          referenceInformation = this@with.referenceInformation,
          molecularWeight = this@with.molecularWeight.takeUnless { it.isEmpty() },
          structure = this@with.structure,
          code = this@with.code.takeUnless { it.isEmpty() },
          name = this@with.name.takeUnless { it.isEmpty() },
          relationship = this@with.relationship.takeUnless { it.isEmpty() },
