...
internal data class DeviceDefinitionUdiDeviceIdentifierMarketDistributionSurrogate(
  ...
  public var subJurisdiction: KotlinString?,
...
internal data class DeviceDefinitionUdiDeviceIdentifierSurrogate(
  ...
  public var extension: MutableList<Extension>? = null,
  public var modifierExtension: MutableList<Extension>? = null,
  public var deviceIdentifier: KotlinString?,
  public var _deviceIdentifier: Element? = null,
  public var issuer: KotlinString?,
  public var _issuer: Element? = null,
  public var jurisdiction: KotlinString?,
    ...
    public fun fromModel(
      ...
      with(model) {
        DeviceDefinitionUdiDeviceIdentifierSurrogate(
          ...
          marketDistribution = this@with.marketDistribution.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(
      ...
      with(model) {
        DeviceDefinitionClassificationSurrogate(
          ...
          modifierExtension = this@with.modifierExtension.takeUnless { it.isEmpty() },
          type = this@with.type,
          justification = this@with.justification.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(model: DeviceDefinition.ConformsTo): DeviceDefinitionConformsToSurrogate =
      with(model) {
        DeviceDefinitionConformsToSurrogate(
          ...
          version =
            this@with.version.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _version =
            this@with.version
              .map { it.toElement() }
              ...
              ?.map { it ?: Element() }
              ?.toMutableList(),
          source = this@with.source.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(
      ...
      with(model) {
        DeviceDefinitionPackagingDistributorSurrogate(
          ...
          name = this@with.name?.value,
          _name = this@with.name?.toElement(),
          organizationReference = this@with.organizationReference.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(model: DeviceDefinition.Packaging): DeviceDefinitionPackagingSurrogate =
      with(model) {
        DeviceDefinitionPackagingSurrogate(
          ...
          count = this@with.count?.value,
          _count = this@with.count?.toElement(),
          distributor = this@with.distributor.takeUnless { it.isEmpty() },
          udiDeviceIdentifier = this@with.udiDeviceIdentifier.takeUnless { it.isEmpty() },
    ...
    public fun fromModel(model: DeviceDefinition.Guideline): DeviceDefinitionGuidelineSurrogate =
      with(model) {
        DeviceDefinitionGuidelineSurrogate(
          ...
          warning = this@with.warning.takeUnless { it.isEmpty() },
...
internal data class DeviceDefinitionCorrectiveActionSurrogate(
  ...
  public var extension: MutableList<Extension>? = null,
  public var modifierExtension: MutableList<Extension>? = null,
  public var recall: KotlinBoolean?,
...
internal data class DeviceDefinitionSurrogate(
  ...
  public fun toModel(): DeviceDefinition =
    DeviceDefinition(
      ...
      language =
        Code.ofNullable(
          this@DeviceDefinitionSurrogate.language,
          this@DeviceDefinitionSurrogate._language,
        ),
    ...
    public fun fromModel(model: DeviceDefinition): DeviceDefinitionSurrogate =
      with(model) {
        DeviceDefinitionSurrogate(
          ...
          udiDeviceIdentifier = this@with.udiDeviceIdentifier.takeUnless { it.isEmpty() },
          regulatoryIdentifier = this@with.regulatoryIdentifier.takeUnless { it.isEmpty() },
          partNumber = this@with.partNumber?.value,
          _partNumber = this@with.partNumber?.toElement(),
          manufacturer = this@with.manufacturer,
          deviceName = this@with.deviceName.takeUnless { it.isEmpty() },
          ...
          conformsTo = this@with.conformsTo.takeUnless { it.isEmpty() },
          hasPart = this@with.hasPart.takeUnless { it.isEmpty() },
          packaging = this@with.packaging.takeUnless { it.isEmpty() },
          version = this@with.version.takeUnless { it.isEmpty() },
          safety = this@with.safety.takeUnless { it.isEmpty() },
          shelfLifeStorage = this@with.shelfLifeStorage.takeUnless { it.isEmpty() },
          languageCode = this@with.languageCode.takeUnless { it.isEmpty() },
          ...
          material = this@with.material.takeUnless { it.isEmpty() },
          productionIdentifierInUDI =
          ...
          chargeItem = this@with.chargeItem.takeUnless { it.isEmpty() },
        )
      }
  }
}
