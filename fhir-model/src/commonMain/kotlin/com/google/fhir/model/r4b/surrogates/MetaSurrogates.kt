...
internal data class MetaSurrogate(
  ...
  public fun toModel(): Meta =
    Meta(
      ...
      versionId = Id.ofNullable(this@MetaSurrogate.versionId, this@MetaSurrogate._versionId),
      lastUpdated =
        Instant.ofNullable(
          FhirDateTime.fromString(this@MetaSurrogate.lastUpdated),
          this@MetaSurrogate._lastUpdated,
        ),
      source = Uri.ofNullable(this@MetaSurrogate.source, this@MetaSurrogate._source),
      profile =
    ...
    public fun fromModel(model: Meta): MetaSurrogate =
      with(model) {
        MetaSurrogate(
          ...
          tag = this@with.tag.takeUnless { it.isEmpty() },
        )
      }
  }
}
