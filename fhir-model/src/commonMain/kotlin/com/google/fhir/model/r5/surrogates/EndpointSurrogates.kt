    ...
    public fun fromModel(model: Endpoint.Payload): EndpointPayloadSurrogate =
      with(model) {
        EndpointPayloadSurrogate(
          ...
          mimeType =
            this@with.mimeType.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _mimeType =
            this@with.mimeType
              .map { it.toElement() }
...
internal data class EndpointSurrogate(
  ...
  public fun toModel(): Endpoint =
    Endpoint(
      ...
      implicitRules =
        Uri.ofNullable(this@EndpointSurrogate.implicitRules, this@EndpointSurrogate._implicitRules),
      language = Code.ofNullable(this@EndpointSurrogate.language, this@EndpointSurrogate._language),
      ...
      name = R5String.ofNullable(this@EndpointSurrogate.name, this@EndpointSurrogate._name),
      description =
        R5String.ofNullable(
          this@EndpointSurrogate.description,
          this@EndpointSurrogate._description,
        ),
    ...
    public fun fromModel(model: Endpoint): EndpointSurrogate =
      with(model) {
        EndpointSurrogate(
          ...
          connectionType = this@with.connectionType.takeUnless { it.isEmpty() },
          name = this@with.name?.value,
          _name = this@with.name?.toElement(),
          description = this@with.description?.value,
          _description = this@with.description?.toElement(),
          environmentType = this@with.environmentType.takeUnless { it.isEmpty() },
          managingOrganization = this@with.managingOrganization,
          contact = this@with.contact.takeUnless { it.isEmpty() },
          period = this@with.period,
          payload = this@with.payload.takeUnless { it.isEmpty() },
          address = this@with.address.value!!,
          _address = this@with.address.toElement(),
          `header` =
            this@with.`header`.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _header =
            this@with.`header`
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
