...
internal data class MedicationDispenseSubstitutionSurrogate(
  ...
  public var extension: MutableList<Extension>? = null,
  public var modifierExtension: MutableList<Extension>? = null,
  public var wasSubstituted: KotlinBoolean?,
    ...
    public fun fromModel(
      ...
      with(model) {
        MedicationDispenseSubstitutionSurrogate(
          ...
          responsibleParty = this@with.responsibleParty.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class MedicationDispenseSurrogate(
  ...
  public fun toModel(): MedicationDispense =
    MedicationDispense(
      ...
      status =
        Enumeration.of(
          com.google.fhir.model.r4.MedicationDispense.MedicationDispenseStatus.fromCode(
            this@MedicationDispenseSurrogate.status!!
          ),
          this@MedicationDispenseSurrogate._status,
        ),
      statusReason =
        MedicationDispense.StatusReason?.fromNullable(
    ...
    public fun fromModel(model: MedicationDispense): MedicationDispenseSurrogate =
      with(model) {
        MedicationDispenseSurrogate(
          ...
          authorizingPrescription = this@with.authorizingPrescription.takeUnless { it.isEmpty() },
          type = this@with.type,
          ...
          `receiver` = this@with.`receiver`.takeUnless { it.isEmpty() },
          note = this@with.note.takeUnless { it.isEmpty() },
          dosageInstruction = this@with.dosageInstruction.takeUnless { it.isEmpty() },
          substitution = this@with.substitution,
          detectedIssue = this@with.detectedIssue.takeUnless { it.isEmpty() },
          eventHistory = this@with.eventHistory.takeUnless { it.isEmpty() },
        )
      }
  }
}
