...
internal data class ProcedureSurrogate(
  ...
  public fun toModel(): Procedure =
    Procedure(
      ...
      implicitRules =
        Uri.ofNullable(
          this@ProcedureSurrogate.implicitRules,
          this@ProcedureSurrogate._implicitRules,
        ),
      language =
        Code.ofNullable(this@ProcedureSurrogate.language, this@ProcedureSurrogate._language),
      ...
      status =
        Enumeration.of(
          com.google.fhir.model.r4.Procedure.ProcedureStatus.fromCode(
      ...
      performed =
        Procedure.Performed?.fromNullable(
          DateTime.ofNullable(
    ...
    public fun fromModel(model: Procedure): ProcedureSurrogate =
      with(model) {
        ProcedureSurrogate(
          ...
          report = this@with.report.takeUnless { it.isEmpty() },
          complication = this@with.complication.takeUnless { it.isEmpty() },
          complicationDetail = this@with.complicationDetail.takeUnless { it.isEmpty() },
          followUp = this@with.followUp.takeUnless { it.isEmpty() },
          note = this@with.note.takeUnless { it.isEmpty() },
          focalDevice = this@with.focalDevice.takeUnless { it.isEmpty() },
          usedReference = this@with.usedReference.takeUnless { it.isEmpty() },
          usedCode = this@with.usedCode.takeUnless { it.isEmpty() },
        )
      }
  }
}
