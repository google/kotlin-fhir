...
internal data class ContractContentDefinitionSurrogate(
  ...
  public var publicationStatus: KotlinString?,
  ...
  public fun toModel(): Contract.ContentDefinition =
    Contract.ContentDefinition(
      ...
      publicationStatus =
        Enumeration.of(
          com.google.fhir.model.r4.Contract.ContractPublicationStatus.fromCode(
    ...
    public fun fromModel(model: Contract.Term.SecurityLabel): ContractTermSecurityLabelSurrogate =
      with(model) {
        ContractTermSecurityLabelSurrogate(
          ...
          extension = this@with.extension.takeUnless { it.isEmpty() },
          modifierExtension = this@with.modifierExtension.takeUnless { it.isEmpty() },
          number = this@with.number.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _number =
            this@with.number
              .map { it.toElement() }
          ...
          control = this@with.control.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class ContractTermOfferSurrogate(
  ...
  public fun toModel(): Contract.Term.Offer =
    Contract.Term.Offer(
      ...
      text =
        R4String.ofNullable(
          this@ContractTermOfferSurrogate.text,
          this@ContractTermOfferSurrogate._text,
        ),
      linkId =
        if (
    ...
    public fun fromModel(model: Contract.Term.Offer): ContractTermOfferSurrogate =
      with(model) {
        ContractTermOfferSurrogate(
          ...
          party = this@with.party.takeUnless { it.isEmpty() },
          topic = this@with.topic,
          type = this@with.type,
          decision = this@with.decision,
          decisionMode = this@with.decisionMode.takeUnless { it.isEmpty() },
          answer = this@with.answer.takeUnless { it.isEmpty() },
          text = this@with.text?.value,
          _text = this@with.text?.toElement(),
          linkId = this@with.linkId.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _linkId =
            this@with.linkId
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
...
internal data class ContractTermAssetValuedItemSurrogate(
  ...
  public fun toModel(): Contract.Term.Asset.ValuedItem =
    Contract.Term.Asset.ValuedItem(
      ...
      entity =
        Contract.Term.Asset.ValuedItem.Entity?.fromNullable(
...
internal data class ContractTermAssetSurrogate(
  ...
  public fun toModel(): Contract.Term.Asset =
    Contract.Term.Asset(
      ...
      text =
        R4String.ofNullable(
          this@ContractTermAssetSurrogate.text,
          this@ContractTermAssetSurrogate._text,
        ),
      linkId =
        if (
    ...
    public fun fromModel(model: Contract.Term.Asset): ContractTermAssetSurrogate =
      with(model) {
        ContractTermAssetSurrogate(
          ...
          typeReference = this@with.typeReference.takeUnless { it.isEmpty() },
          subtype = this@with.subtype.takeUnless { it.isEmpty() },
          relationship = this@with.relationship,
          context = this@with.context.takeUnless { it.isEmpty() },
          condition = this@with.condition?.value,
          _condition = this@with.condition?.toElement(),
          periodType = this@with.periodType.takeUnless { it.isEmpty() },
          period = this@with.period.takeUnless { it.isEmpty() },
          usePeriod = this@with.usePeriod.takeUnless { it.isEmpty() },
          text = this@with.text?.value,
          _text = this@with.text?.toElement(),
          linkId = this@with.linkId.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _linkId =
          ...
          _securityLabelNumber =
            this@with.securityLabelNumber
              ...
              ?.map { it ?: Element() }
              ?.toMutableList(),
          valuedItem = this@with.valuedItem.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class ContractTermActionSurrogate(
  ...
  public fun toModel(): Contract.Term.Action =
    Contract.Term.Action(
      ...
      occurrence =
        Contract.Term.Action.Occurrence?.fromNullable(
          DateTime.ofNullable(
    ...
    public fun fromModel(model: Contract.Term.Action): ContractTermActionSurrogate =
      with(model) {
        ContractTermActionSurrogate(
          ...
          type = this@with.type,
          subject = this@with.subject.takeUnless { it.isEmpty() },
          ...
          contextLinkId =
            this@with.contextLinkId.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _contextLinkId =
          ...
          requester = this@with.requester.takeUnless { it.isEmpty() },
          requesterLinkId =
          ...
          _requesterLinkId =
            this@with.requesterLinkId
              ...
              ?.map { it ?: Element() }
              ?.toMutableList(),
          performerType = this@with.performerType.takeUnless { it.isEmpty() },
          ...
          reason = this@with.reason.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _reason =
            this@with.reason
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ...
              ?.toMutableList(),
          reasonLinkId =
            this@with.reasonLinkId.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _reasonLinkId =
...
internal data class ContractTermSurrogate(
  ...
  public fun toModel(): Contract.Term =
    Contract.Term(
      ...
      topic =
        Contract.Term.Topic?.fromNullable(
      ...
      text = R4String.ofNullable(this@ContractTermSurrogate.text, this@ContractTermSurrogate._text),
    ...
    public fun fromModel(model: Contract.Term): ContractTermSurrogate =
      with(model) {
        ContractTermSurrogate(
          ...
          asset = this@with.asset.takeUnless { it.isEmpty() },
          action = this@with.action.takeUnless { it.isEmpty() },
          group = this@with.group.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(model: Contract.Signer): ContractSignerSurrogate =
      with(model) {
        ContractSignerSurrogate(
          ...
          signature = this@with.signature.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class ContractSurrogate(
  ...
  public fun toModel(): Contract =
    Contract(
      ...
      implicitRules =
        Uri.ofNullable(this@ContractSurrogate.implicitRules, this@ContractSurrogate._implicitRules),
      language = Code.ofNullable(this@ContractSurrogate.language, this@ContractSurrogate._language),
      ...
      url = Uri.ofNullable(this@ContractSurrogate.url, this@ContractSurrogate._url),
      version =
        R4String.ofNullable(this@ContractSurrogate.version, this@ContractSurrogate._version),
      status =
        this@ContractSurrogate.status?.let {
          Enumeration.of(
            com.google.fhir.model.r4.Contract.ContractStatus.fromCode(it!!),
      ...
      instantiatesUri =
        Uri.ofNullable(
          this@ContractSurrogate.instantiatesUri,
          this@ContractSurrogate._instantiatesUri,
        ),
      ...
      name = R4String.ofNullable(this@ContractSurrogate.name, this@ContractSurrogate._name),
      title = R4String.ofNullable(this@ContractSurrogate.title, this@ContractSurrogate._title),
      subtitle =
        R4String.ofNullable(this@ContractSurrogate.subtitle, this@ContractSurrogate._subtitle),
      alias =
      ...
      legallyBinding =
        Contract.LegallyBinding?.fromNullable(
    ...
    public fun fromModel(model: Contract): ContractSurrogate =
      with(model) {
        ContractSurrogate(
          ...
          subject = this@with.subject.takeUnless { it.isEmpty() },
          authority = this@with.authority.takeUnless { it.isEmpty() },
          domain = this@with.domain.takeUnless { it.isEmpty() },
          site = this@with.site.takeUnless { it.isEmpty() },
          ...
          alias = this@with.alias.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _alias =
            this@with.alias
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
          ...
          term = this@with.term.takeUnless { it.isEmpty() },
          supportingInfo = this@with.supportingInfo.takeUnless { it.isEmpty() },
          relevantHistory = this@with.relevantHistory.takeUnless { it.isEmpty() },
          signer = this@with.signer.takeUnless { it.isEmpty() },
          friendly = this@with.friendly.takeUnless { it.isEmpty() },
          legal = this@with.legal.takeUnless { it.isEmpty() },
          rule = this@with.rule.takeUnless { it.isEmpty() },
