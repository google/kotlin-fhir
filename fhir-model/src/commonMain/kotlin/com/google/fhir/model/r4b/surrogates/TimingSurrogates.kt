...
internal data class TimingRepeatSurrogate(
  ...
  public fun toModel(): Timing.Repeat =
    Timing.Repeat(
      ...
      bounds =
        Timing.Repeat.Bounds?.fromNullable(
          this@TimingRepeatSurrogate.boundsDuration,
          this@TimingRepeatSurrogate.boundsRange,
          this@TimingRepeatSurrogate.boundsPeriod,
        ),
      count =
        PositiveInt.ofNullable(this@TimingRepeatSurrogate.count, this@TimingRepeatSurrogate._count),
      countMax =
        PositiveInt.ofNullable(
          this@TimingRepeatSurrogate.countMax,
          this@TimingRepeatSurrogate._countMax,
        ),
      duration =
        Decimal.ofNullable(
          this@TimingRepeatSurrogate.duration,
          this@TimingRepeatSurrogate._duration,
        ),
      durationMax =
        Decimal.ofNullable(
          this@TimingRepeatSurrogate.durationMax,
          this@TimingRepeatSurrogate._durationMax,
        ),
      durationUnit =
        Code.ofNullable(
          this@TimingRepeatSurrogate.durationUnit,
          this@TimingRepeatSurrogate._durationUnit,
        ),
      frequency =
        PositiveInt.ofNullable(
          this@TimingRepeatSurrogate.frequency,
          this@TimingRepeatSurrogate._frequency,
        ),
      frequencyMax =
        PositiveInt.ofNullable(
      ...
      period =
        Decimal.ofNullable(this@TimingRepeatSurrogate.period, this@TimingRepeatSurrogate._period),
      periodMax =
        Decimal.ofNullable(
          this@TimingRepeatSurrogate.periodMax,
          this@TimingRepeatSurrogate._periodMax,
        ),
      periodUnit =
        Code.ofNullable(
          this@TimingRepeatSurrogate.periodUnit,
      ...
      `when` =
        if (this@TimingRepeatSurrogate.`when` == null && this@TimingRepeatSurrogate._when == null) {
          ...
          (this@TimingRepeatSurrogate.`when`
            ...
            .map { (value, element) ->
              Enumeration.of(
                value.let { com.google.fhir.model.r4b.Timing.EventTiming.fromCode(it!!) },
                element,
              )
            }
            .toMutableList()
      ...
      offset =
        UnsignedInt.ofNullable(
          this@TimingRepeatSurrogate.offset,
          this@TimingRepeatSurrogate._offset,
        ),
    )
        ...
        TimingRepeatSurrogate(
          ...
          _dayOfWeek =
            this@with.dayOfWeek
              ...
              ?.toMutableList(),
          timeOfDay =
            this@with.timeOfDay.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _timeOfDay =
            this@with.timeOfDay
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ...
              ?.toMutableList(),
          `when` =
            this@with.`when`.map { it.value.getCode() }.toMutableList().takeUnless { it.isEmpty() },
          _when =
            this@with.`when`
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
    ...
    public fun fromModel(model: Timing): TimingSurrogate =
      with(model) {
        TimingSurrogate(
          ...
          event =
            this@with.event.map { it.value.toString() }.toMutableList().takeUnless { it.isEmpty() },
          _event =
            this@with.event
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
