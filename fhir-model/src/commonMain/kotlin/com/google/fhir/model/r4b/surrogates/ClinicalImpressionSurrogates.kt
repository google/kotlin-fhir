...
internal data class ClinicalImpressionSurrogate(
  ...
  public fun toModel(): ClinicalImpression =
    ClinicalImpression(
      ...
      effective =
        ClinicalImpression.Effective?.fromNullable(
          DateTime.ofNullable(
    ...
    public fun fromModel(model: ClinicalImpression): ClinicalImpressionSurrogate =
      with(model) {
        ClinicalImpressionSurrogate(
          ...
          problem = this@with.problem.takeUnless { it.isEmpty() },
          investigation = this@with.investigation.takeUnless { it.isEmpty() },
          protocol =
            this@with.protocol.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _protocol =
            this@with.protocol
              .map { it.toElement() }
          ...
          finding = this@with.finding.takeUnless { it.isEmpty() },
          prognosisCodeableConcept = this@with.prognosisCodeableConcept.takeUnless { it.isEmpty() },
          prognosisReference = this@with.prognosisReference.takeUnless { it.isEmpty() },
