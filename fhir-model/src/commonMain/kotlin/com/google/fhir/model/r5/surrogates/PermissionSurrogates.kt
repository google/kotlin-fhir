    ...
    public fun fromModel(model: Permission.Justification): PermissionJustificationSurrogate =
      with(model) {
        PermissionJustificationSurrogate(
          ...
          evidence = this@with.evidence.takeUnless { it.isEmpty() },
        )
      }
  }
}
    ...
    public fun fromModel(model: Permission.Rule.Data): PermissionRuleDataSurrogate =
      with(model) {
        PermissionRuleDataSurrogate(
          ...
          security = this@with.security.takeUnless { it.isEmpty() },
    ...
    public fun fromModel(model: Permission.Rule): PermissionRuleSurrogate =
      with(model) {
        PermissionRuleSurrogate(
          ...
          limit = this@with.limit.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class PermissionSurrogate(
  ...
  public fun toModel(): Permission =
    Permission(
      ...
      implicitRules =
        Uri.ofNullable(
          this@PermissionSurrogate.implicitRules,
          this@PermissionSurrogate._implicitRules,
        ),
      language =
        Code.ofNullable(this@PermissionSurrogate.language, this@PermissionSurrogate._language),
    ...
    public fun fromModel(model: Permission): PermissionSurrogate =
      with(model) {
        PermissionSurrogate(
          ...
          date =
            this@with.date.map { it.value.toString() }.toMutableList().takeUnless { it.isEmpty() },
          _date =
            this@with.date
              .map { it.toElement() }
