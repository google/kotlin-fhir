...
internal data class SearchParameterSurrogate(
  ...
  public fun toModel(): SearchParameter =
    SearchParameter(
      ...
      versionAlgorithm =
        SearchParameter.VersionAlgorithm?.fromNullable(
          R5String.ofNullable(
      ...
      title =
        R5String.ofNullable(
          this@SearchParameterSurrogate.title,
          this@SearchParameterSurrogate._title,
        ),
      derivedFrom =
    ...
    public fun fromModel(model: SearchParameter): SearchParameterSurrogate =
      with(model) {
        SearchParameterSurrogate(
          ...
          base = this@with.base.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _base =
            this@with.base
              .map { it.toElement() }
          ...
          target = this@with.target.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _target =
            this@with.target
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
