...
internal data class GenomicStudyAnalysisInputSurrogate(
  ...
  public fun toModel(): GenomicStudy.Analysis.Input =
    GenomicStudy.Analysis.Input(
      ...
      generatedBy =
        GenomicStudy.Analysis.Input.GeneratedBy?.fromNullable(
    ...
    public fun fromModel(model: GenomicStudy.Analysis): GenomicStudyAnalysisSurrogate =
      with(model) {
        GenomicStudyAnalysisSurrogate(
          ...
          methodType = this@with.methodType.takeUnless { it.isEmpty() },
          changeType = this@with.changeType.takeUnless { it.isEmpty() },
          ...
          specimen = this@with.specimen.takeUnless { it.isEmpty() },
          date = this@with.date?.value?.toString(),
          _date = this@with.date?.toElement(),
          note = this@with.note.takeUnless { it.isEmpty() },
          protocolPerformed = this@with.protocolPerformed,
          regionsStudied = this@with.regionsStudied.takeUnless { it.isEmpty() },
          regionsCalled = this@with.regionsCalled.takeUnless { it.isEmpty() },
...
internal data class GenomicStudySurrogate(
  ...
  public fun toModel(): GenomicStudy =
    GenomicStudy(
      ...
      implicitRules =
        Uri.ofNullable(
          this@GenomicStudySurrogate.implicitRules,
          this@GenomicStudySurrogate._implicitRules,
        ),
      language =
        Code.ofNullable(this@GenomicStudySurrogate.language, this@GenomicStudySurrogate._language),
    ...
    public fun fromModel(model: GenomicStudy): GenomicStudySurrogate =
      with(model) {
        GenomicStudySurrogate(
          ...
          interpreter = this@with.interpreter.takeUnless { it.isEmpty() },
          ...
          analysis = this@with.analysis.takeUnless { it.isEmpty() },
        )
      }
  }
}
