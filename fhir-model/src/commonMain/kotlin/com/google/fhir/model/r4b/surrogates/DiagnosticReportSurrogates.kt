...
internal data class DiagnosticReportSurrogate(
  ...
  public fun toModel(): DiagnosticReport =
    DiagnosticReport(
      ...
      language =
        Code.ofNullable(
          this@DiagnosticReportSurrogate.language,
          this@DiagnosticReportSurrogate._language,
        ),
      ...
      status =
        Enumeration.of(
          com.google.fhir.model.r4b.DiagnosticReport.DiagnosticReportStatus.fromCode(
      ...
      effective =
        DiagnosticReport.Effective?.fromNullable(
          DateTime.ofNullable(
    ...
    public fun fromModel(model: DiagnosticReport): DiagnosticReportSurrogate =
      with(model) {
        DiagnosticReportSurrogate(
          ...
          resultsInterpreter = this@with.resultsInterpreter.takeUnless { it.isEmpty() },
          specimen = this@with.specimen.takeUnless { it.isEmpty() },
          result = this@with.result.takeUnless { it.isEmpty() },
          imagingStudy = this@with.imagingStudy.takeUnless { it.isEmpty() },
          media = this@with.media.takeUnless { it.isEmpty() },
          conclusion = this@with.conclusion?.value,
          _conclusion = this@with.conclusion?.toElement(),
          conclusionCode = this@with.conclusionCode.takeUnless { it.isEmpty() },
          presentedForm = this@with.presentedForm.takeUnless { it.isEmpty() },
        )
      }
  }
}
