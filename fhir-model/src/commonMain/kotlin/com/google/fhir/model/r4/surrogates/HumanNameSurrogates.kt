...
internal data class HumanNameSurrogate(
  ...
  public fun toModel(): HumanName =
    HumanName(
      ...
      text = R4String.ofNullable(this@HumanNameSurrogate.text, this@HumanNameSurrogate._text),
      family = R4String.ofNullable(this@HumanNameSurrogate.family, this@HumanNameSurrogate._family),
      given =
    ...
    public fun fromModel(model: HumanName): HumanNameSurrogate =
      with(model) {
        HumanNameSurrogate(
          ...
          given = this@with.given.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _given =
            this@with.given
              .map { it.toElement() }
              ...
              ?.map { it ?: Element() }
              ?.toMutableList(),
          prefix = this@with.prefix.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _prefix =
            this@with.prefix
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ?.map { it ?: Element() }
              ?.toMutableList(),
          suffix = this@with.suffix.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _suffix =
            this@with.suffix
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
