...
internal data class MedicinalProductNameSurrogate(
  ...
  public var extension: MutableList<Extension>? = null,
  public var modifierExtension: MutableList<Extension>? = null,
  public var productName: KotlinString?,
    ...
    public fun fromModel(model: MedicinalProduct.Name): MedicinalProductNameSurrogate =
      with(model) {
        MedicinalProductNameSurrogate(
          ...
          namePart = this@with.namePart.takeUnless { it.isEmpty() },
          countryLanguage = this@with.countryLanguage.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class MedicinalProductSpecialDesignationSurrogate(
  ...
  public fun toModel(): MedicinalProduct.SpecialDesignation =
    MedicinalProduct.SpecialDesignation(
      ...
      indication =
        MedicinalProduct.SpecialDesignation.Indication?.fromNullable(
...
internal data class MedicinalProductSurrogate(
  ...
  public fun toModel(): MedicinalProduct =
    MedicinalProduct(
      ...
      language =
        Code.ofNullable(
          this@MedicinalProductSurrogate.language,
          this@MedicinalProductSurrogate._language,
        ),
    ...
    public fun fromModel(model: MedicinalProduct): MedicinalProductSurrogate =
      with(model) {
        MedicinalProductSurrogate(
          ...
          productClassification = this@with.productClassification.takeUnless { it.isEmpty() },
          marketingStatus = this@with.marketingStatus.takeUnless { it.isEmpty() },
          pharmaceuticalProduct = this@with.pharmaceuticalProduct.takeUnless { it.isEmpty() },
          packagedMedicinalProduct = this@with.packagedMedicinalProduct.takeUnless { it.isEmpty() },
          attachedDocument = this@with.attachedDocument.takeUnless { it.isEmpty() },
          masterFile = this@with.masterFile.takeUnless { it.isEmpty() },
          contact = this@with.contact.takeUnless { it.isEmpty() },
          clinicalTrial = this@with.clinicalTrial.takeUnless { it.isEmpty() },
          name = this@with.name.takeUnless { it.isEmpty() },
          crossReference = this@with.crossReference.takeUnless { it.isEmpty() },
          manufacturingBusinessOperation =
            this@with.manufacturingBusinessOperation.takeUnless { it.isEmpty() },
          specialDesignation = this@with.specialDesignation.takeUnless { it.isEmpty() },
        )
      }
  }
}
