    ...
    public fun fromModel(
      ...
      with(model) {
        MolecularSequenceQualityRocSurrogate(
          ...
          extension = this@with.extension.takeUnless { it.isEmpty() },
          modifierExtension = this@with.modifierExtension.takeUnless { it.isEmpty() },
          score = this@with.score.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _score =
            this@with.score
              .map { it.toElement() }
              ...
              ?.map { it ?: Element() }
              ?.toMutableList(),
          numTP = this@with.numTP.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _numTP =
            this@with.numTP
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ?.map { it ?: Element() }
              ?.toMutableList(),
          numFP = this@with.numFP.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _numFP =
            this@with.numFP
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ?.map { it ?: Element() }
              ?.toMutableList(),
          numFN = this@with.numFN.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _numFN =
            this@with.numFN
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ...
              ?.toMutableList(),
          precision =
            this@with.precision.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _precision =
            this@with.precision
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
              ...
              ?.toMutableList(),
          sensitivity =
            this@with.sensitivity.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _sensitivity =
            this@with.sensitivity
              ...
              ?.toMutableList(),
          fMeasure =
            this@with.fMeasure.map { it.value!! }.toMutableList().takeUnless { it.isEmpty() },
          _fMeasure =
            this@with.fMeasure
              .map { it.toElement() }
              .takeUnless { it.all { it == null } }
...
internal data class MolecularSequenceRepositorySurrogate(
  ...
  public fun toModel(): MolecularSequence.Repository =
    MolecularSequence.Repository(
      ...
      type =
        Enumeration.of(
          com.google.fhir.model.r4.MolecularSequence.RepositoryType.fromCode(
    ...
    public fun fromModel(model: MolecularSequence): MolecularSequenceSurrogate =
      with(model) {
        MolecularSequenceSurrogate(
          ...
          variant = this@with.variant.takeUnless { it.isEmpty() },
          observedSeq = this@with.observedSeq?.value,
          _observedSeq = this@with.observedSeq?.toElement(),
          quality = this@with.quality.takeUnless { it.isEmpty() },
          readCoverage = this@with.readCoverage?.value,
          _readCoverage = this@with.readCoverage?.toElement(),
          repository = this@with.repository.takeUnless { it.isEmpty() },
          pointer = this@with.pointer.takeUnless { it.isEmpty() },
          structureVariant = this@with.structureVariant.takeUnless { it.isEmpty() },
        )
      }
  }
}
