...
internal data class ConsentPolicyBasisSurrogate(
  ...
  public fun toModel(): Consent.PolicyBasis =
    Consent.PolicyBasis(
      ...
      url =
        Url.ofNullable(this@ConsentPolicyBasisSurrogate.url, this@ConsentPolicyBasisSurrogate._url),
    )
...
internal data class ConsentVerificationSurrogate(
  ...
  public var extension: MutableList<Extension>? = null,
  public var modifierExtension: MutableList<Extension>? = null,
  public var verified: KotlinBoolean?,
    ...
    public fun fromModel(model: Consent.Provision): ConsentProvisionSurrogate =
      with(model) {
        ConsentProvisionSurrogate(
          ...
          purpose = this@with.purpose.takeUnless { it.isEmpty() },
          documentType = this@with.documentType.takeUnless { it.isEmpty() },
          resourceType = this@with.resourceType.takeUnless { it.isEmpty() },
          code = this@with.code.takeUnless { it.isEmpty() },
          dataPeriod = this@with.dataPeriod,
          `data` = this@with.`data`.takeUnless { it.isEmpty() },
          expression = this@with.expression,
          provision = this@with.provision.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class ConsentSurrogate(
  ...
  public fun toModel(): Consent =
    Consent(
      ...
      implicitRules =
        Uri.ofNullable(this@ConsentSurrogate.implicitRules, this@ConsentSurrogate._implicitRules),
      language = Code.ofNullable(this@ConsentSurrogate.language, this@ConsentSurrogate._language),
      ...
      date =
        Date.ofNullable(
          FhirDate.fromString(this@ConsentSurrogate.date),
          this@ConsentSurrogate._date,
        ),
    ...
    public fun fromModel(model: Consent): ConsentSurrogate =
      with(model) {
        ConsentSurrogate(
          ...
          period = this@with.period,
          grantor = this@with.grantor.takeUnless { it.isEmpty() },
          grantee = this@with.grantee.takeUnless { it.isEmpty() },
          manager = this@with.manager.takeUnless { it.isEmpty() },
          controller = this@with.controller.takeUnless { it.isEmpty() },
          sourceAttachment = this@with.sourceAttachment.takeUnless { it.isEmpty() },
          sourceReference = this@with.sourceReference.takeUnless { it.isEmpty() },
          regulatoryBasis = this@with.regulatoryBasis.takeUnless { it.isEmpty() },
          policyBasis = this@with.policyBasis,
          policyText = this@with.policyText.takeUnless { it.isEmpty() },
          verification = this@with.verification.takeUnless { it.isEmpty() },
