...
internal data class ClinicalUseDefinitionIndicationSurrogate(
  ...
  public fun toModel(): ClinicalUseDefinition.Indication =
    ClinicalUseDefinition.Indication(
      ...
      duration =
        ClinicalUseDefinition.Indication.Duration?.fromNullable(
    ...
    public fun fromModel(
      ...
      with(model) {
        ClinicalUseDefinitionIndicationSurrogate(
          ...
          undesirableEffect = this@with.undesirableEffect.takeUnless { it.isEmpty() },
          applicability = this@with.applicability,
          otherTherapy = this@with.otherTherapy.takeUnless { it.isEmpty() },
        )
      }
    ...
    public fun fromModel(
      ...
      with(model) {
        ClinicalUseDefinitionInteractionSurrogate(
          ...
          extension = this@with.extension.takeUnless { it.isEmpty() },
          modifierExtension = this@with.modifierExtension.takeUnless { it.isEmpty() },
          interactant = this@with.interactant.takeUnless { it.isEmpty() },
          type = this@with.type,
          effect = this@with.effect,
          incidence = this@with.incidence,
          management = this@with.management.takeUnless { it.isEmpty() },
        )
      }
  }
}
