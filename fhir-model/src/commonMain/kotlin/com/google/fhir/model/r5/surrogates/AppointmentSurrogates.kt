...
internal data class AppointmentSurrogate(
  ...
  public fun toModel(): Appointment =
    Appointment(
      ...
      implicitRules =
        Uri.ofNullable(
          this@AppointmentSurrogate.implicitRules,
          this@AppointmentSurrogate._implicitRules,
        ),
      language =
        Code.ofNullable(this@AppointmentSurrogate.language, this@AppointmentSurrogate._language),
      ...
      description =
        R5String.ofNullable(
          this@AppointmentSurrogate.description,
          this@AppointmentSurrogate._description,
        ),
    ...
    public fun fromModel(model: Appointment): AppointmentSurrogate =
      with(model) {
        AppointmentSurrogate(
          ...
          `class` = this@with.`class`.takeUnless { it.isEmpty() },
          serviceCategory = this@with.serviceCategory.takeUnless { it.isEmpty() },
          serviceType = this@with.serviceType.takeUnless { it.isEmpty() },
          ...
          replaces = this@with.replaces.takeUnless { it.isEmpty() },
          virtualService = this@with.virtualService.takeUnless { it.isEmpty() },
          supportingInformation = this@with.supportingInformation.takeUnless { it.isEmpty() },
          ...
          requestedPeriod = this@with.requestedPeriod.takeUnless { it.isEmpty() },
          slot = this@with.slot.takeUnless { it.isEmpty() },
          account = this@with.account.takeUnless { it.isEmpty() },
          ...
          patientInstruction = this@with.patientInstruction.takeUnless { it.isEmpty() },
          ...
          recurrenceTemplate = this@with.recurrenceTemplate.takeUnless { it.isEmpty() },
        )
      }
  }
}
