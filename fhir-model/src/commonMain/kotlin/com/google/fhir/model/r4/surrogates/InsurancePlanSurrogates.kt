    ...
    public fun fromModel(
      ...
      with(model) {
        InsurancePlanPlanSpecificCostBenefitCostSurrogate(
          ...
          qualifiers = this@with.qualifiers.takeUnless { it.isEmpty() },
          `value` = this@with.`value`,
        )
      }
  }
    ...
    public fun fromModel(model: InsurancePlan.Plan): InsurancePlanPlanSurrogate =
      with(model) {
        InsurancePlanPlanSurrogate(
          ...
          network = this@with.network.takeUnless { it.isEmpty() },
          generalCost = this@with.generalCost.takeUnless { it.isEmpty() },
          specificCost = this@with.specificCost.takeUnless { it.isEmpty() },
        )
      }
  }
}
...
internal data class InsurancePlanSurrogate(
  ...
  public fun toModel(): InsurancePlan =
    InsurancePlan(
      ...
      language =
        Code.ofNullable(
          this@InsurancePlanSurrogate.language,
          this@InsurancePlanSurrogate._language,
        ),
      ...
      name =
        R4String.ofNullable(this@InsurancePlanSurrogate.name, this@InsurancePlanSurrogate._name),
      alias =
        if (
    ...
    public fun fromModel(model: InsurancePlan): InsurancePlanSurrogate =
      with(model) {
        InsurancePlanSurrogate(
          ...
          plan = this@with.plan.takeUnless { it.isEmpty() },
        )
      }
  }
}
