    ...
    public fun fromModel(
      ...
      with(model) {
        ResearchStudyComparisonGroupSurrogate(
          ...
          intendedExposure = this@with.intendedExposure.takeUnless { it.isEmpty() },
...
internal data class ResearchStudySurrogate(
  ...
  public fun toModel(): ResearchStudy =
    ResearchStudy(
      ...
      language =
        Code.ofNullable(
          this@ResearchStudySurrogate.language,
          this@ResearchStudySurrogate._language,
        ),
      ...
      url = Uri.ofNullable(this@ResearchStudySurrogate.url, this@ResearchStudySurrogate._url),
      identifier = this@ResearchStudySurrogate.identifier ?: mutableListOf(),
      version =
        R5String.ofNullable(
          this@ResearchStudySurrogate.version,
          this@ResearchStudySurrogate._version,
        ),
      name =
        R5String.ofNullable(this@ResearchStudySurrogate.name, this@ResearchStudySurrogate._name),
      title =
        R5String.ofNullable(this@ResearchStudySurrogate.title, this@ResearchStudySurrogate._title),
    ...
    public fun fromModel(model: ResearchStudy): ResearchStudySurrogate =
      with(model) {
        ResearchStudySurrogate(
          ...
          label = this@with.label.takeUnless { it.isEmpty() },
          protocol = this@with.protocol.takeUnless { it.isEmpty() },
          ...
          keyword = this@with.keyword.takeUnless { it.isEmpty() },
          region = this@with.region.takeUnless { it.isEmpty() },
          ...
          classifier = this@with.classifier.takeUnless { it.isEmpty() },
          associatedParty = this@with.associatedParty.takeUnless { it.isEmpty() },
          progressStatus = this@with.progressStatus.takeUnless { it.isEmpty() },
          whyStopped = this@with.whyStopped,
          recruitment = this@with.recruitment,
          comparisonGroup = this@with.comparisonGroup.takeUnless { it.isEmpty() },
          objective = this@with.objective.takeUnless { it.isEmpty() },
          outcomeMeasure = this@with.outcomeMeasure.takeUnless { it.isEmpty() },
